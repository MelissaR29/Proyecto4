...
def modulador(bits, fc, mpp):
    ...
    for i, bit in enumerate(bits):
        ...
        if (bit == 0) and (n1%2 == 0) and (n2%2 == 0):
        ...
        if (bit == 1) and (n1%2 == 0) and (n2%2 != 0):
            senal_Tx[i*mpp : (i+1)*mpp] = portadoracos*1
            moduladora[i*mpp : (i+1)*mpp] = 1
            n1 = n1 + 1
            n2 = n2 + 1
        ...
        if (bit == 0) and (n1%2 != 0) and (n2%2 == 0):
            senal_Tx[i*mpp : (i+1)*mpp] = portadorasin*-3
            moduladora[i*mpp : (i+1)*mpp] = 0
            n1 = n1 + 1
            n2 = n2 + 1
          
        if (bit == 0) and (n1%2 != 0) and (n2%2 != 0):
            senal_Tx[i*mpp : (i+1)*mpp] = portadorasin*-1
            moduladora[i*mpp : (i+1)*mpp] = 0
            n1 = n1 + 1
            n2 = n2 + 1
    ...
    portadoraF = portadoracos + portadorasin
    
    return Pm, portadoraF, moduladora, senal_Tx 
...
def demodulador(senal_Rx, portadoraF, mpp):
    '''Un método que simula un bloque demodulador
    ...
    Es = np.sum(portadoraF * portadoraF)

    # Demodulación
    for i in range(N):
        # Producto interno de dos funciones
        producto = senal_Rx[i*mpp : (i+1)*mpp] * portadoraF
        senal_demodulada[i*mpp : (i+1)*mpp] = producto
        Ep = np.sum(producto) 
...
# Simulación del sistema de comunicaciones con modulación 16QAM
...
senal_Tx, Pm, portadoraF, moduladora = modulador(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador(senal_Rx, portadoraF, mpp)
...
# 4.2. - Análisis: Estacionaridad y ergodicidad

# En este aaprtado se hacen pruebas de estacionaridad y ergodicidad a la señal modulada senal_Tx, 
...
# Formas de onda
X_t = np.empty((16, len(t_x)))   # 16 funciones del tiempo x(t) 
...
for i in A:
    ...
    x3 = i * np.cos(2*(np.pi)*fc*t_x) +  -i*np.sin(2*(np.pi)*fc*t_x)
    x4 = -i * np.cos(2*(np.pi)*fc*t_x) +  -i*np.sin(2*(np.pi)*fc*t_x)
    X_t[i,:] = x1
    X_t[i+1,:] = x2
    X_t[i+2,:] = x3
    X_t[i+3,:] = x4
    plt.plot(t_x,x1, lw=4)
    plt.plot(t_x, x2, lw=4)
    plt.plot(t_x, x3, lw=4)
    plt.plot(t_x, x4, lw=4)

# Promedio de las 16 realizaciones en cada instante 
...
# Determinar y graficar la densidad espectral de potencia para la señal modulada senal_Tx.
...
# Aquí se procese a hacer el plot de la gráfica de la densidad espectral. 
